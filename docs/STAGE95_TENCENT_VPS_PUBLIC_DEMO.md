...
阶段 96 已完成公网 IP、安全组和外部浏览器访问验收，详见 `docs/STAGE96_TENCENT_VPS_BROWSER_ACCEPTANCE.md`。当前入口为 `http://49.232.104.202/`，通过 TCP 80 访问；域名、HTTPS、限流、备份、监控和更严格的 BYOK 出站策略仍是演示版上线前的后续门禁。Docker 构建成功本身不等于公网可用，本阶段的浏览器证据才完成了这一门禁。
