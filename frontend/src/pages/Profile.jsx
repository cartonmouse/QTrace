...
export default function Profile() {
  ...
  if (loading) {
    return (
      <div className={cn(PAGE_CLASS, "qtrace-profile-page space-y-4")}>
  ...
  if (!hasData) {
    ...
    return (
      <div className={cn(PAGE_CLASS, "qtrace-profile-page")}>
        <div className="qtrace-profile-heading">
          <div className="qtrace-profile-index">00 / PERSONAL MEMORY</div>
          <div className="qtrace-profile-title">个人画像</div>
          <p>把每一次回答沉淀成下一次训练可以使用的证据。</p>
        </div>
        <Card className="qtrace-profile-surface qtrace-profile-empty mt-5 overflow-hidden border-primary/20 bg-[linear-gradient(135deg,rgba(245,158,11,0.12),rgba(20,184,166,0.08))] dark:bg-[linear-gradient(135deg,rgba(245,158,11,0.16),rgba(8,145,178,0.12))]">
          <CardContent className="qtrace-profile-empty-content p-8 md:p-10">
            <div className="qtrace-profile-empty-copy max-w-2xl">
              <Badge className="qtrace-profile-badge mb-4 bg-primary/12 text-primary">还没有训练数据</Badge>
              <div className="qtrace-profile-empty-title text-2xl font-semibold leading-tight md:text-4xl">
                先积累几轮回答，再让页面开始提炼真正的重点。
              </div>
              <div className="qtrace-profile-empty-lede mt-4 text-sm leading-7 text-dim md:text-base">
                开始面试后，系统会逐步把你的弱项、强项、答题模式和领域变化沉淀下来。等第一批数据形成，页面会自动切到驾驶舱视图。
              </div>
            </div>

            <div className="qtrace-profile-section-label mt-7 text-xs font-medium text-dim">选一个方式开始第一场面试</div>
            <div className="qtrace-profile-route-grid mt-3 grid gap-3 md:grid-cols-3">
              {startOptions.map((option) => {
                ...
                <button
                  ...
                  className="qtrace-profile-route-card group relative flex flex-col gap-3 rounded-2xl border border-border/80 bg-card/70 p-4 text-left transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-card hover:shadow-lg hover:shadow-primary/5"
                >
                  <div className="flex items-center justify-between">
                    <div className="qtrace-profile-route-icon flex h-10 w-10 items-center justify-center rounded-xl border border-border/80 bg-background text-dim transition-colors duration-300 group-hover:border-primary/30 group-hover:bg-primary/10 group-hover:text-primary">
                      <Icon size={20} />
                    </div>
                  ...
                  <div>
                    <div className="qtrace-profile-route-title text-[15px] font-semibold tracking-tight text-text">{title}</div>
                    <div className="qtrace-profile-route-desc mt-1 text-xs leading-5 text-dim">{desc}</div>
                  </div>
                  <div className="qtrace-profile-route-hint text-[11px] font-medium text-primary/80">{hint}</div>
                </button>
                );
              })}
            </div>
  ...
  return (
    <div className={cn(PAGE_CLASS, "qtrace-profile-page")}>
      <div className="qtrace-profile-heading animate-fade-in">
        <div className="qtrace-profile-index">00 / PERSONAL MEMORY</div>
        <div className="qtrace-profile-title">个人画像</div>
      ...
      {visitDelta && (
        <Card className="qtrace-profile-surface mt-5 animate-fade-in-up [animation-delay:0.02s] border-primary/25 bg-[linear-gradient(135deg,rgba(245,158,11,0.05),transparent)]">
      ...
      {dueReviews.length > 0 && (
        <Card className="qtrace-profile-surface mt-5 animate-fade-in-up [animation-delay:0.03s] border-primary/25 bg-[linear-gradient(135deg,rgba(245,158,11,0.06),transparent)]">
          <CardContent className="p-4 md:p-5">
      ...
      <Card className="qtrace-profile-surface mt-5 animate-fade-in-up [animation-delay:0.04s]">
        <CardContent className="p-5 md:p-6">
          ...
          <div className="qtrace-profile-stat-grid mt-6 grid grid-cols-2 gap-x-6 gap-y-5 md:grid-cols-4">
            <div>
      ...
      {/* ═══ 能力特征 (大卡, 知识轴) ═══ */}
      <Card className="qtrace-profile-surface mt-5 animate-fade-in-up [animation-delay:0.08s]">
        <CardContent className="p-5 md:p-6">
      ...
      {/* ═══ 表现特征 (大卡, 表现轴) ═══ */}
      <Card className="qtrace-profile-surface mt-5 animate-fade-in-up [animation-delay:0.12s]">
        <CardContent className="p-5 md:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-baseline gap-3">
      ...
      {scoreHistory.length >= 2 && (
        <Card className="qtrace-profile-surface mt-5 animate-fade-in-up [animation-delay:0.2s]">
          <CardContent className="p-5 md:p-6">
            <SectionHeader
              icon={<TrendingUp size={18} />}
              title="成长趋势"
