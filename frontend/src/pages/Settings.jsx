...
// 网络错误可能包含服务商回显的凭据片段；设置页只展示可诊断的摘要，不展示敏感值。
function redactSettingsError(value) {
  const message = value instanceof Error ? value.message : String(value || "未知错误");
  return message
    .replace(/Bearer\s+[A-Za-z0-9._~-]+/gi, "Bearer [已隐藏]")
    .replace(/(?:sk|key|token)[-_]?[A-Za-z0-9]{8,}/gi, "[已隐藏凭据]")
    .slice(0, 280);
}
...
export default function Settings() {
  ...
  useEffect(() => {
    getSettings()
      ...
      .catch((err) => setError("加载设置失败: " + redactSettingsError(err)))
  ...
  const handleExport = async (kind) => {
    ...
    try {
      ...
      setMigrationError("导出失败：" + redactSettingsError(err));
    } finally {
      setExporting(null);
    }
  };
  ...
  const handleImportConfirm = async () => {
    ...
    try {
      ...
      setMigrationError("导入失败：" + redactSettingsError(err));
    } finally {
      setImportBusy(false);
    }
  };
  ...
  const handleTestLLM = async () => {
    ...
    try {
      ...
      setLlmTest(r.ok ? { status: "ok" } : { status: "fail", error: redactSettingsError(r.error) });
    } catch (err) {
      setLlmTest({ status: "fail", error: redactSettingsError(err) });
    }
  };
  ...
  const handleTestEmbedding = async () => {
    ...
    try {
      ...
      setEmbTest(r.ok ? { status: "ok" } : { status: "fail", error: redactSettingsError(r.error) });
    } catch (err) {
      setEmbTest({ status: "fail", error: redactSettingsError(err) });
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError("保存失败: " + redactSettingsError(err));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleRebuildIndex = async () => {
    ...
    try {
      await rebuildEmbeddingIndex({
        ...
        onError: (e) => setReindexError("重建失败: " + redactSettingsError(e)),
      });
    } catch (err) {
      setReindexError("重建失败: " + redactSettingsError(err));
    } finally {
      setReindexing(false);
      setReindexProgress(null);
    }
  ...
  if (loading) {
    return (
      <div className="qtrace-settings-page flex-1 flex items-center justify-center text-dim">
        <Loader2 size={20} className="animate-spin" />
        <span className="sr-only">正在加载模型设置</span>
      </div>
    );
  }
  ...
  const renderTestRow = (test, onTest) => (
    <div className="qtrace-settings-test-row mt-6 flex flex-wrap items-center gap-3 border-t border-border/40 pt-5">
      <Button
        variant="outline"
        onClick={onTest}
        disabled={test?.status === "testing"}
        className="qtrace-settings-action h-10 rounded-xl"
      >
        {test?.status === "testing" ? (
          <>
      ...
      {test?.status === "ok" ? (
        <span role="status" data-qtrace-state="ok" className="qtrace-settings-result qtrace-settings-result-ok flex items-center gap-1.5 text-[13px] text-emerald-500">
          <Check size={15} /> 连接正常
        </span>
      ) : test?.status === "fail" ? (
        <span role="alert" data-qtrace-state="fail" className="qtrace-settings-result qtrace-settings-result-fail flex items-start gap-1.5 text-[13px] text-red-500">
          <XCircle size={15} className="mt-0.5 shrink-0" /> {test.error || "连接失败"}
        </span>
      ) : test?.status === "testing" ? (
        <span role="status" data-qtrace-state="testing" className="qtrace-settings-result qtrace-settings-result-testing text-[12px] text-dim">正在发起最小请求…</span>
      ) : (
        <span data-qtrace-state="idle" className="qtrace-settings-result text-[12px] text-dim">用当前填写的配置发一个最小请求，验证是否可用</span>
      )}
    </div>
  );
  ...
  return (
    <div className="qtrace-settings-page flex-1 w-full max-w-[1080px] mx-auto px-4 pt-6 pb-0 md:px-7 md:pt-8">
      <div className="qtrace-settings-heading mb-7">
        <div className="qtrace-settings-index">MODEL CONTROL / 01</div>
        <div className="qtrace-settings-title text-2xl md:text-[28px] font-display font-bold">模型设置</div>
        <div className="qtrace-settings-lede text-sm text-dim mt-1">配置 LLM、Embedding 与训练参数；每次测试和保存都会留下明确状态。</div>
      </div>

      <div className="qtrace-settings-status-board" aria-live="polite">
        <div className="qtrace-settings-status-cell">
          <span className="qtrace-settings-status-label">LLM / CONNECTION</span>
          <strong data-qtrace-state={llmTest?.status || "idle"}>
            {llmTest?.status === "testing" ? "测试中" : llmTest?.status === "ok" ? "已连接" : llmTest?.status === "fail" ? "连接失败" : "待测试"}
          </strong>
          <small>{llmTest?.error || "不会展示 API Key；只显示连接结果"}</small>
        </div>
        <div className="qtrace-settings-status-cell">
          <span className="qtrace-settings-status-label">EMBEDDING / {embBackend === "local" ? "LOCAL" : embBackend === "api" ? "API" : "AUTO"}</span>
          <strong data-qtrace-state={embTest?.status || "idle"}>
            {embTest?.status === "testing" ? "测试中" : embTest?.status === "ok" ? "已连接" : embTest?.status === "fail" ? "连接失败" : "待测试"}
          </strong>
          <small>{embTest?.error || (needsReindex ? "配置已变化，保存后需要更新向量索引" : "本地模型启用后可在此查看测试结果")}</small>
        </div>
        <div className="qtrace-settings-status-cell">
          <span className="qtrace-settings-status-label">CONFIG / SAVE</span>
          <strong data-qtrace-state={saving ? "testing" : saved ? "ok" : error ? "fail" : "idle"}>
            {saving ? "保存中" : saved ? "已保存" : error ? "保存失败" : "待保存"}
          </strong>
          <small>{lastReindexAt ? `向量索引 ${lastReindexAt.replace("T", " ").slice(0, 16)}` : "修改后点击底部保存"}</small>
        </div>
      </div>

      {error && <div role="alert" className="qtrace-settings-global-error">{error}</div>}

      <div className="grid gap-5 lg:grid-cols-[220px_1fr]">
        {/* Left Tab Rail */}
        <nav className="qtrace-settings-rail lg:sticky lg:top-4 lg:self-start">
        ...
        {/* Right Content Pane */}
        <div className="qtrace-settings-content min-w-0 space-y-5">
        {/* LLM Provider */}
        <Card ref={llmRef} data-tab-id="llm" className="qtrace-settings-card overflow-hidden border-border/40 bg-card/40 scroll-mt-4">
        ...
        {/* Embedding */}
        <Card ref={embeddingRef} data-tab-id="embedding" className="qtrace-settings-card overflow-hidden border-border/40 bg-card/40 scroll-mt-4">
          <CardContent className="p-5 md:p-7">
            <div className="flex items-center gap-2 mb-1">
      ...
      <div className="qtrace-settings-savebar sticky bottom-0 z-10 -mx-4 mt-6 border-t border-border/40 bg-background/85 px-4 py-3 backdrop-blur-md md:-mx-7 md:px-7">
