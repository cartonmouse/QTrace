...
export async function loadRegistrationConfig({
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      // QTrace intentionally keeps registration open without a separate
      // public config endpoint. Preserve TechSpar's UI while treating a
      // missing optional endpoint as the local default.
